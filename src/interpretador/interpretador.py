...
# Import robusto: tenta import relativo (quando executado como módulo).
# Se o script for executado diretamente (sem pacote pai), a importação relativa
# falhará — então adicionamos a pasta `src` ao sys.path e usamos import absoluto.
try:
...
if __name__ == "__main__":
    # lê "instrucoes.txt", carrega na Memoria interna e lista as instruções decodificadas.
    arquivo = os.path.join(os.path.dirname(__file__), "instrucoes.txt")
    interp = InterpretadorDeInstrucoes()

    if os.path.exists(arquivo):
        interp.carregar_arquivo(arquivo)
        interp.listar_programa()
    else:
