...
INSTRUCOES = {
    ...
    "loadi": "00011000",  # Carrega imediato
    "halt": "11111111"
}
...
def montar_instrucao(asm: str) -> str:
    ...
    # separar por vírgula ou espaço (remove vírgulas)
    ...
    if mnemonic in ["add", "sub", "xor", "or", "and", "asl", "asr", "lsl", "lsr"]:
        # Exemplo: add r3, r2, r1 -> add rc=r3 ra=r2 rb=r1
        rc = _parse_operand(args[0])
        ra = _parse_operand(args[1])
        rb = _parse_operand(args[2])
    elif mnemonic == "zeros":
        # Exemplo: zeros r1 -> zeros rc=r1
        rc = _parse_operand(args[0])
    elif mnemonic == "passa":
        # Exemplo: passa r1, r2 -> passa rc=r1 ra=r2
        rc = _parse_operand(args[0])
        ra = _parse_operand(args[1])
    elif mnemonic == "lcl_msb":
        rc = _parse_operand(args[0])
        const16 = _parse_operand(args[1])
    elif mnemonic == "lcl_lsb":
        rc = _parse_operand(args[0])
        const16 = _parse_operand(args[1])
    elif mnemonic == "load":
        rc = _parse_operand(args[0])
